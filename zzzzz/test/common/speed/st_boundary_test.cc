...
TEST(StBoundaryTest, boundary_range) {
  // read config file
  std::string config_file("config/json/planning_conf.json");
  PlanningGflags::setPlanningConfig(config_file);
  RefereneLineGflags::setReferenceLineConfig(
      PlanningGflags::local_route_path);
  DecisionGflags::setDecisionConfig(PlanningGflags::decision_path);
