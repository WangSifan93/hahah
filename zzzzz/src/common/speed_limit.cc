...
double SpeedLimit::GetSpeedLimitByS(const double s) const {
  ...
  auto it_lower =
      std::lower_bound(speed_limit_points_.begin(), speed_limit_points_.end(),
                       s - MinDeltaS, compare_s);
  if (it_lower == speed_limit_points_.end()) {
    return (it_lower - 1)->second;
  } else if (it_lower == speed_limit_points_.begin()) {
    return it_lower->second;
  } else {
    return ::math::lerp((it_lower - 1)->second, (it_lower - 1)->first,
                        it_lower->second, it_lower->first, s);
  }
}
