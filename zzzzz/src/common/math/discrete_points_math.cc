...
void DiscretePointsMath::FiltFilt(std::vector<double>& signal) {
  const uint8_t order = 20;
  std::vector<double> filter(order + 1, 1.0 / (order + 1));

  const size_t point_size = signal.size();
  auto convolve = [](const std::vector<double>& a,
                     const std::vector<double>& b) {
    size_t n = a.size();
    size_t m = b.size();
    size_t total_size = n + m - 1;
  ...
  // 正向
  std::vector<double> temp_signal = convolve(signal, filter);

  // 反向
  std::vector<double> reversed_signal(temp_signal.rbegin(), temp_signal.rend());
  std::vector<double> reversed_filtered_signal =
      convolve(reversed_signal, filter);
  std::reverse(reversed_filtered_signal.begin(),
               reversed_filtered_signal.end());
  signal = std::vector<double>(
      reversed_filtered_signal.begin() + order,
      reversed_filtered_signal.begin() + order + point_size);
}
