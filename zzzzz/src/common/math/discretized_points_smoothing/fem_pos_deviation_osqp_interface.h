...
const double kInvalidWeight = 0.0;
...
class FemPosDeviationOsqpInterface {
 public:
  ...
  void set_last_ref_points(
      const std::vector<std::pair<double, double>>& last_ref_points) {
  ...
  void set_side_bounds_around_refs(
      const std::vector<std::pair<double, double>>& bounds_around_refs) {
    side_bounds_around_refs_ = bounds_around_refs;
  }
  ...
  void set_weight_last_ref_deviation(const double weight_last_ref_deviation) {
    weight_last_ref_deviation_ = weight_last_ref_deviation;
  }

  void SetLastRefIndexRange(const std::pair<uint32_t, uint32_t>& ref_index) {
    last_ref_index_ = ref_index;
  }
  ...
  bool Solve(const bool& diff_bound = false);
 ...
 private:
  ...
  std::vector<std::pair<double, double>> last_ref_points_;
  std::pair<uint32_t, uint32_t> last_ref_index_;
  std::vector<double> bounds_around_refs_;
  std::vector<std::pair<double, double>>
      side_bounds_around_refs_;  // upper bound is different from lower bound
  ...
  double weight_last_ref_deviation_ = 1.0e5;

  bool add_last_ref_cost_ = false;
