...
#include "apps/planning/src/common/math/discretized_points_smoothing/fem_pos_deviation_osqp_interface.h"
...
bool FemPosDeviationOsqpInterface::Solve(const bool& is_different_bound) {
  // Sanity Check
  if (ref_points_.empty()) {
    AERROR << "reference points empty, solver early terminates";
    return false;
  }
  AINFO << " last ref size: " << last_ref_points_.size();
  if (last_ref_points_.empty()) {
    last_ref_points_ = ref_points_;
    weight_last_ref_deviation_ = kInvalidWeight;
    last_ref_index_ = {0, 0};
    add_last_ref_cost_ = false;
  } else if (last_ref_points_.size() != ref_points_.size()) {
    AERROR << "last ref size != current ref size";
    return false;
  } else {
    if (last_ref_index_.first == last_ref_index_.second ||
        last_ref_index_.first >= ref_points_.size() ||
        last_ref_index_.second >= ref_points_.size()) {
      add_last_ref_cost_ = false;
    } else {
      add_last_ref_cost_ = true;
    }
  }
  AINFO << "add_last_ref_cost_: " << add_last_ref_cost_
        << " last_ref_index_: " << last_ref_index_.first
        << " sec: " << last_ref_index_.second;

  if (!is_different_bound && ref_points_.size() != bounds_around_refs_.size()) {
    AERROR << "ref_points and bounds size not equal, solver early terminates";
    return false;
  }

  if (is_different_bound &&
      ref_points_.size() != side_bounds_around_refs_.size()) {
    AERROR << "ref_points and bounds size not equal, solver early terminates";
    return false;
  }
  ...
  if (ref_points_.size() > (std::size_t)std::numeric_limits<int>::max()) {
  ...
  if (P_indptr.size() == 0) {
    AERROR << "P_indptr is empty, stop smooth solve.";
    return false;
  }
  ...
  CalculateAffineConstraint(&A_data, &A_indices, &A_indptr, &lower_bounds,
                            &upper_bounds, is_different_bound);
...
void FemPosDeviationOsqpInterface::CalculateKernel(
  ...
  for (int col = 0; col < 2; ++col) {
    double weight_last_ref_deviation = kInvalidWeight;
    if (add_last_ref_cost_ &&
        col <= 2 * static_cast<int>(last_ref_index_.second) + 1 &&
        col >= 2 * static_cast<int>(last_ref_index_.first)) {
      weight_last_ref_deviation = weight_last_ref_deviation_;
    }
    columns[col].emplace_back(
        col, weight_fem_pos_deviation_ + weight_path_length_ +
                 weight_ref_deviation_ + weight_last_ref_deviation);
    ++col_num;
  }
  ...
  for (int col = 2; col < 4; ++col) {
    ...
    columns[col].emplace_back(
        col, 5.0 * weight_fem_pos_deviation_ + 2.0 * weight_path_length_ +
                 weight_ref_deviation_ + weight_last_ref_deviation);
    ++col_num;
  }
  ...
  for (int point_index = 2; point_index < second_point_from_last_index;
    ...
    double weight_last_ref_deviation = kInvalidWeight;
    if (add_last_ref_cost_ &&
        point_index <= static_cast<int>(last_ref_index_.second) &&
        point_index >= static_cast<int>(last_ref_index_.first)) {
      weight_last_ref_deviation = weight_last_ref_deviation_;
    }
    for (int col = 0; col < 2; ++col) {
      col_index += col;
  ...
  // use if condition replace CHECK_EQ(col_num, num_of_variables_);
  if (col_num != num_of_variables_) {
    AERROR << "col_num: " << col_num
...
void FemPosDeviationOsqpInterface::CalculateOffset(std::vector<c_float>* q) {
  for (int i = 0; i < num_of_points_; ++i) {
    double weight_last_ref_deviation = kInvalidWeight;
    if (add_last_ref_cost_ && i <= static_cast<int>(last_ref_index_.second) &&
        i >= static_cast<int>(last_ref_index_.first)) {
      weight_last_ref_deviation = weight_last_ref_deviation_;
    }
    const auto& ref_point_xy = ref_points_[i];
    const auto& last_ref_point_xy = last_ref_points_[i];
    q->push_back(-2.0 * weight_ref_deviation_ * ref_point_xy.first +
                 (-2.0 * weight_last_ref_deviation * last_ref_point_xy.first));
    q->push_back(-2.0 * weight_ref_deviation_ * ref_point_xy.second +
                 (-2.0 * weight_last_ref_deviation * last_ref_point_xy.second));
  }
}
...
void FemPosDeviationOsqpInterface::CalculateAffineConstraint(
    ...
    std::vector<c_float>* upper_bounds, const bool& diff_bound) {
  int ind_A = 0;
  ...
  if (!diff_bound) {
    ...
    const double lon_bound = 1.0;
    for (int i = 0; i < num_of_points_; ++i) {
      const auto& ref_point_xy = ref_points_[i];
      upper_bounds->push_back(ref_point_xy.first + lon_bound);
      upper_bounds->push_back(ref_point_xy.second +
                              side_bounds_around_refs_[i].first);
      lower_bounds->push_back(ref_point_xy.first - lon_bound);
      lower_bounds->push_back(ref_point_xy.second -
                              side_bounds_around_refs_[i].second);
    }
  }
}
