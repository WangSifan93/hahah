...
#include "apps/planning/src/common/math/discretized_points_smoothing/fem_pos_deviation_sqp_osqp_interface.h"
...
bool FemPosDeviationSmoother::Solve(
    ...
    const std::pair<uint32_t, uint32_t>& ref_index_range,
    const std::vector<double>& bounds, std::vector<double>* opt_x,
    std::vector<double>* opt_y) {
  if (config_.apply_curvature_constraint()) {
    if (config_.use_sqp()) {
      if (!SqpWithOsqp(raw_point2d, bounds, opt_x, opt_y)) {
        AINFO << "SqpWithOsqp FAIL..";
        return QpWithOsqp(raw_point2d, last_raw_point2d, ref_index_range,
                          bounds, opt_x, opt_y);
      }
      return true;
    }
  }
  return QpWithOsqp(raw_point2d, last_raw_point2d, ref_index_range, bounds,
                    opt_x, opt_y);
}
...
bool FemPosDeviationSmoother::QpWithOsqp(
  ...
  solver.set_weight_last_ref_deviation(config_.weight_last_ref_deviation());
  ...
  solver.set_last_ref_points(last_raw_point2d);
  solver.SetLastRefIndexRange(ref_index_range);
...
bool FemPosDeviationSmoother::QpWithOsqp(
  ...
  solver.set_weight_fem_pos_deviation(config_.weight_fem_pos_deviation());
  solver.set_weight_path_length(config_.weight_path_length());
  solver.set_weight_ref_deviation(config_.weight_fem_pos_deviation() * 1.0e6);
  ...
  solver.set_ref_points(raw_point2d);
  solver.set_side_bounds_around_refs(bounds);
