...
class STBoundary : public ::math::Polygon2d {
 public:
  ...
  explicit STBoundary(
      ...
      const std::string& id = "", bool is_accurate_boundary = false);
  ...
  double GetLowerSByT(const double& t) const;
  double GetUpperSByT(const double& t) const;
  ...
  std::vector<STPoint>* mutable_upper_points() { return &upper_points_; }
  std::vector<STPoint>* mutable_lower_points() { return &lower_points_; }
