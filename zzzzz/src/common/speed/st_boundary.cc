...
#include "apps/planning/src/common/util/print_debug_info.h"
...
STBoundary::STBoundary(
    ...
    const std::string& obs_id, bool is_accurate_boundary) {
...
STBoundary STBoundary::CreateInstanceAccurate(
    const std::vector<STPoint>& lower_points,
    const std::vector<STPoint>& upper_points, const std::string& obs_id) {
  if (lower_points.size() != upper_points.size() || lower_points.size() < 2) {
    return STBoundary();
  }
  ...
  return STBoundary(point_pairs, obs_id, true);
}
...
bool STBoundary::GetBoundarySRange(const double curr_time, double* s_upper,
  ...
  *s_lower = lower_points_[left].s() +
  ...
  // ego position should be considered glk
  const double init_s = 200.0;
  const double speed_lon_decision_horizon = 200.0;
  // const double lowest_s = -10000.0;
  *s_upper = std::fmax(*s_upper, 0.0);
  *s_upper = std::fmin(*s_upper, speed_lon_decision_horizon + init_s);
  *s_lower = std::fmax(*s_lower, 0.0);
  *s_lower = std::fmin(*s_lower, speed_lon_decision_horizon + init_s);
  return true;
}
...
bool STBoundary::IsPointInBoundary(const STPoint& st_point) const {
  ...
  const double check_upper =
      ::math::CrossProd(st_point, upper_points_[left], upper_points_[right]);
  const double check_lower =
      ::math::CrossProd(st_point, lower_points_[left], lower_points_[right]);
...
double STBoundary::GetLowerSByT(const double& t) const {
  if (lower_points_.size() < kNumMin) {
    return std::numeric_limits<double>::max();
  }

  auto it_lower = std::lower_bound(
      lower_points_.begin(), lower_points_.end(), t - kMinDeltaT,
      [](const STPoint& point, const double t_cur) {
        return point.t() < t_cur;
      });
  if (it_lower == lower_points_.begin()) {
    return lower_points_.front().s();
  } else if (it_lower == lower_points_.end()) {
    return lower_points_.back().s();
  } else {
    return ::math::lerp(it_lower->s(), it_lower->t(), (it_lower - 1)->s(),
                        (it_lower - 1)->t(), t);
  }
}

double STBoundary::GetUpperSByT(const double& t) const {
  if (upper_points_.size() < kNumMin) {
    return std::numeric_limits<double>::max();
  }

  auto it_lower = std::lower_bound(
      upper_points_.begin(), upper_points_.end(), t - kMinDeltaT,
      [](const STPoint& point, const double t_cur) {
        return point.t() < t_cur;
      });
  if (it_lower == upper_points_.begin()) {
    return lower_points_.front().s();
  } else if (it_lower == upper_points_.end()) {
    return upper_points_.back().s();
  } else {
    return ::math::lerp(it_lower->s(), it_lower->t(), (it_lower - 1)->s(),
                        (it_lower - 1)->t(), t);
  }
