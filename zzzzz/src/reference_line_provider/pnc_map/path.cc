...
bool FindLaneSegment(const MapPathPoint& p1, const MapPathPoint& p2,
  ...
  for (const auto& wp1 : p1.lane_waypoints()) {
    for (const auto& wp2 : p2.lane_waypoints()) {
      if (wp1.lane->lane().id().id() == wp2.lane->lane().id().id() &&
          wp1.s < wp2.s) {
...
std::string LaneWaypoint::DebugString() const {
  ...
  std::string ss =
      "id = " + lane->lane().id().id() + "   s = " + std::to_string(s);
  return ss;
}

zark::hdmap_new::LaneBoundaryAttribute_Type LeftBoundaryType(
    const LaneWaypoint& waypoint) {
  auto res = LaneBoundaryAttribute_Type::
      LaneBoundaryAttribute_Type_LANEBOUNDARY_TYPE_UNKNOWN;
  if (!waypoint.lane) {
    return res;
  }
  auto left_bound_info = hdmap::GetLaneBoundaryById(
      hdmap::HDMapUtil::BaseMapPtr(), waypoint.lane->lane().left_boundary_id());
  if (left_bound_info == nullptr) {
    return res;
  }
  for (const auto& type : left_bound_info->lane_boundary().boundary_attr()) {
    if ((type.distance_info().distance_to_vehicle_cm() -
         waypoint.lane->lane().distance_info().distance_to_vehicle_cm()) *
            kCentimeterToMetre <=
        waypoint.s) {
      return type.type();
    }
...
zark::hdmap_new::LaneBoundaryAttribute_Type RightBoundaryType(
    const LaneWaypoint& waypoint) {
  auto res = LaneBoundaryAttribute_Type::
      LaneBoundaryAttribute_Type_LANEBOUNDARY_TYPE_UNKNOWN;
  if (!waypoint.lane) {
  ...
  auto right_bound_info = hdmap::GetLaneBoundaryById(
      hdmap::HDMapUtil::BaseMapPtr(), waypoint.lane->lane().left_boundary_id());
  if (right_bound_info == nullptr) {
    return res;
  }
  for (const auto& type : right_bound_info->lane_boundary().boundary_attr()) {
    if ((type.distance_info().distance_to_vehicle_cm() -
         waypoint.lane->lane().distance_info().distance_to_vehicle_cm()) *
            kCentimeterToMetre <=
        waypoint.s) {
...
LaneWaypoint LeftNeighborWaypoint(const LaneWaypoint& waypoint) {
  ...
  for (const auto& lane_id :
    ...
    auto lane = GetLaneById(map_ptr, lane_id);
    if (!lane) {
      return neighbor;
    }
    double s = 0.0;
...
std::string LaneSegment::DebugString() const {
  ...
  return " id =" + lane->lane().id().id() +
         "  start_s = " + std::to_string(start_s) +
         "  end_s = " + std::to_string(end_s);
}
...
std::vector<MapPathPoint> MapPathPoint::GetPointsFromLane(
    map_service::zmap::LaneInfoConstPtr lane, const double start_s,
    const double end_s) {
  ...
  for (size_t i = 0; i < lane->points().size(); ++i) {
    if (accumulate_s >= start_s && accumulate_s <= end_s) {
      points.emplace_back(
          ::math::Vec2d(lane->points()[i].x(), lane->points()[i].y()),
          lane->headings()[i], LaneWaypoint(lane, accumulate_s));
    }
    if (i < lane->segments().size()) {
      ...
      if (start_s > accumulate_s && start_s < next_accumulate_s) {
        auto start_point = segment.start() +
                           segment.unit_direction() * (start_s - accumulate_s);
        points.emplace_back(::math::Vec2d(start_point.x(), start_point.y()),
...
std::string MapPathPoint::DebugString() const {
  std::string input_string =
      "x = " + std::to_string(x_) + "   y = " + std::to_string(y_);
  return input_string;
}

std::string Path::DebugString() const {
  std::string input_string =
      "num_points_ point = " + std::to_string(num_points_);
  return input_string;
}

std::string PathOverlap::DebugString() const {
  std::string input_string =
      "object_id" + object_id + "start_s " + std::to_string(start_s);
  return input_string;
}
...
Path::Path(const std::vector<MapPathPoint>& path_points,
           const bool is_side_line)
    : path_points_(path_points), is_side_line_(is_side_line) {
  Init();
}
...
void Path::InitWidth() {
  ...
  double s = 0;
  const double kDefaultLaneWidth = 3.75;  //[m]
  for (int i = 0; i < num_sample_points_; ++i) {
    const MapPathPoint point = GetSmoothPoint(s);
    if (point.lane_waypoints().empty()) {
      lane_left_width_.push_back(kDefaultLaneWidth / 2.0);
      lane_right_width_.push_back(kDefaultLaneWidth / 2.0);

      road_left_width_.push_back(kDefaultLaneWidth / 2.0);
      road_right_width_.push_back(kDefaultLaneWidth / 2.0);
      ...
      hdmap_new::LaneSampleAssociation lane_width =
          waypoint.lane->GetWidth(waypoint.s / kCentimeterToMetre);
      lane_left_width_.push_back(
          lane_width.left_width_cm() * kCentimeterToMetre - waypoint.l);
      lane_right_width_.push_back(
          lane_width.right_width_cm() * kCentimeterToMetre + waypoint.l);

      hdmap_new::LaneSampleAssociation road_width =
          waypoint.lane->GetRoadWidth(waypoint.s / kCentimeterToMetre);
      road_left_width_.push_back(
          road_width.left_width_cm() * kCentimeterToMetre - waypoint.l);
      road_right_width_.push_back(
          road_width.right_width_cm() * kCentimeterToMetre + waypoint.l);
    }
    s += kSampleDistance;
  }
...
MapPathPoint Path::GetSmoothPoint(const InterpolatedIndex& index) const {
  ...
  if (std::abs(index.offset) > kMathEpsilon) {
    const ::math::Vec2d delta = unit_directions_[index.id] * index.offset;
    ...
    if (index.id < num_segments_ && !ref_point.lane_waypoints().empty()) {
      ...
      if (lane_segment.lane != nullptr) {
        for (const auto& lane_waypoint : ref_point.lane_waypoints()) {
          if (lane_waypoint.lane->lane().id().id() ==
...
bool Path::GetNearestPoint(const ::math::Vec2d& point, double* accumulate_s,
...
bool Path::GetProjectionWithHueristicParams(const ::math::Vec2d& point,
...
bool Path::GetHeadingAlongPath(const ::math::Vec2d& point,
...
bool Path::IsOnPath(const ::math::Vec2d& point) const {
  double accumulate_s = 0.0;
  double lateral = 0.0;
...
bool Path::OverlapWith(const ::math::Box2d& box, double width) const {
  ...
  const ::math::Vec2d center = box.center();
