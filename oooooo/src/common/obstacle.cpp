...
void Obstacle::InsertFrame(const DynamicObstacleInfo& dyn_obs,
  ...
  obs_frame.SetIsSpacetime(is_spacetime);
  ...
  if (obs_history_frames_.size() < 2) {
    ...
    const ObstacleFrame* const last_frame = this->GetFrameAt(1);
    VectorPoint vp;
  ...
  const auto& angle = math::Angle16::FromRad(obs_frame.heading());
...
void Obstacle::ConvertDynamicToFloats(const Vec2d& anchor_point,
  ...
  for (std::size_t frame_idx = 0;
    ...
    const auto angle = math::Angle16::FromRad(heading);
    const double cos_angle = math::cos(angle);
    const double sin_angle = math::sin(angle);
    ...
    fp->at(vector_end_idx - dynamic_vector_dim_ + 16u) = 0.0;
...
void Obstacle::ConvertStaticToFloats(const Vec2d& anchor_point,
  ...
  for (std::size_t point_idx = 0u; point_idx < max_size - 1;
    ...
    fp->at(vector_end_idx - static_vector_dim_ + 4u) = 0;
...
std::vector<Vec2d> StationaryObstacle::GetCornerPoints(
  ...
  if (!polygon_.points().empty()) {
    ...
    return GetBoundingBox().GetAllCorners();
  }
}
