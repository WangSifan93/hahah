...
bool Path::SLToXY(const SLPoint &sl_point, Point2d *xy_point,
  ...
  const auto diff_angle = math::Angle16::FromRad(matched_point.theta);
  xy_point->set_x(matched_point.x() + diff_s * math::cos(diff_angle) -
                  math::sin(diff_angle) * sl_point.l);
  xy_point->set_y(matched_point.y() + diff_s * math::sin(diff_angle) +
                  math::cos(diff_angle) * sl_point.l);
  if (ref_point) {
...
bool Path::XYToSL(const math::Box2d &box_2d, SLBoundary *sl_boundary) const {
  return XYToSL(box_2d.GetAllCorners(), sl_boundary);
}
...
bool Path::ComputePathProfile(std::vector<PathPoint> &path_points) {
  ...
  for (size_t index = 0; index < points_size; ++index) {
    ...
    path_points.at(index).theta = calculate_heading(index);
    if (index == 0) {
...
bool Path::XYToSL(std::vector<PathPoint> &path_points) const {
  ...
  while (i < path_points.size()) {
    ...
    const Point2d vec_direct(cos(p0->theta), sin(p0->theta));
...
bool Path::SLToXY(std::vector<PathPoint> &path_points) const {
  ...
  const auto diff_angle = math::Angle16::FromRad(match_point.theta);
  path_points.front().set_x(match_point.x() + diff_s * math::cos(diff_angle) -
                            math::sin(diff_angle) * path_points.front().l);
  path_points.front().set_y(match_point.y() + diff_s * math::sin(diff_angle) +
                            math::cos(diff_angle) * path_points.front().l);
  ...
  while (i < path_points.size()) {
    ...
    const auto dangle = math::Angle16::FromRad(match_point.theta);
    path_points[i].set_x(match_point.x() + ds * math::cos(dangle) -
                         math::sin(dangle) * path_points[i].l);
    path_points[i].set_y(match_point.y() + ds * math::sin(dangle) +
                         math::cos(dangle) * path_points[i].l);
    ++i;
  }
