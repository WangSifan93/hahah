...
absl::Status CheckAndBuildFrenetInput(
  ...
  DCHECK([raw_points]() {
    ...
    return true;
  }()) << ::e2e_noa::DebugString(raw_points);
...
FrenetBox FindBoundingFrenetBoundingBox(
    const std::vector<FrenetCoordinate>& coords) {
...
absl::StatusOr<FrenetBox> FrenetFrame::QueryFrenetBoxAtPointsWithHeading(
  ...
  return FindBoundingFrenetBoundingBox(coords);
}
...
KdTreeFrenetCoordinate::XYToSLWithHeadingDiffLimitImplement(
    const Vec2d& xy, double heading, double max_heading_diff) const {
...
void KdTreeFrenetCoordinate::XYToSLImplement(const Vec2d& xy,
                                             FrenetCoordinate* sl,
...
void BruteForceFrenetCoordinate::XYToSLImplement(const Vec2d& xy,
                                                 FrenetCoordinate* sl,
                                                 Vec2d* normal, int* index,
                                                 double* alpha) const {
  double min_d = std::numeric_limits<double>::infinity();
...
void QtfmEnhancedKdTreeFrenetCoordinate::XYToSLImplement(const Vec2d& xy,
                                                         FrenetCoordinate* sl,
                                                         Vec2d* normal,
                                                         int* index,
                                                         double* alpha) const {
...
absl::StatusOr<BruteForceFrenetCoordinate> BuildBruteForceFrenetFrame(
  ...
  return BruteForceFrenetCoordinate(
...
absl::StatusOr<KdTreeFrenetCoordinate> BuildKdTreeFrenetFrame(
  ...
  segment_matcher = std::make_unique<KdtreeSegmentMatcher>(segments);
  return KdTreeFrenetCoordinate(std::move(points), std::move(s_knots),
...
BuildQtfmEnhancedKdTreeFrenetCoordinate(absl::Span<const Vec2d> raw_points,
  ...
  segment_matcher = std::make_shared<KdtreeSegmentMatcher>(segments);
  ...
  return QtfmEnhancedKdTreeFrenetCoordinate(
      std::move(points), std::move(s_knots), std::move(segment_len_inv),
