...
void LaneManager::update(const PlannerWorldInput &input,
  ...
  const auto ego_pos = e2e_planner_context_->ego_state()->ego_pos();
  const auto ego_theta = e2e_planner_context_->ego_state()->ego_theta();
  const auto start_point_velocity =
      e2e_planner_context_->ego_state()->start_point_velocity();
  const auto &plan_start_point =
      e2e_planner_context_->ego_state()->plan_start_point();
  ...
  auto &lane_change_style =
      e2e_planner_context_->lane_change_command_update()->lane_change_style();

  auto &updated_preferred_lane_path =
      e2e_planner_context_->lane_change_command_update()->preferred_lane_path();
  const auto &new_lc_cmd_state =
      e2e_planner_context_->lane_change_command_update()->new_lc_cmd_state();
  ...
  const auto target_lp_infos_cr = FilterMultipleTargetLanePath(
      psmm, route_sections_info_from_start, aligned_route_navi_info,
      prev_target_lane_path_from_start_, plan_start_point,
      updated_preferred_lane_path, &lp_infos_cr.value());
#if LANE_PATH_DEBUG
