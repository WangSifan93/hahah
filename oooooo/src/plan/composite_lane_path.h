...
class CompositeLanePath {
 public:
  ...
  struct TransitionInfo {
    static constexpr double kDefaultOverlapLength = 50.0;
 ...
 public:
  ...
  CompositeLanePath(const ad_e2e::planning::MapConstPtr& map_ptr,
  ...
  LanePath::LaneIndexWaypoint LanePathEntryIndexPoint(
      int lane_path_index) const {
    return lane_path_transition_points_[lane_path_index].first;
  }
  LanePath::LaneIndexWaypoint LanePathExitIndexPoint(
      int lane_path_index) const {
  ...
  CompositeLanePath Connect(const ad_e2e::planning::MapConstPtr& map_ptr,
                            const CompositeLanePath& subsequent_path) const;
  ...
  class LaneIndexWaypoint {
   public:
    LaneIndexWaypoint() = default;
    LaneIndexWaypoint(const CompositeIndex& index, double lane_fraction)
    ...
    bool operator==(const LaneIndexWaypoint& o) const {
      return index() == o.index() && lane_fraction() == o.lane_fraction();
    }

    template <typename H>
    friend H AbslHashValue(H h, const LaneIndexWaypoint& lip) {
  ...
  bool ContainsLanePoint(
      LanePoint point,
      LaneIndexWaypoint* first_encounter_of_lane_index_point = nullptr,
  ...
  CompositeLanePath BeforeLaneIndexPoint(
 ...
 private:
  ...
  std::vector<
      std::pair<LanePath::LaneIndexWaypoint, LanePath::LaneIndexWaypoint>>
      lane_path_transition_points_;

  std::vector<mapping::ElementId> lane_ids_;
  std::vector<int> cumulative_lane_path_sizes_;
};
