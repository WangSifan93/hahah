...
size_t Problem::SyncToExistingProblem(const VectorXf& x0, Time t0,
  ...
  CHECK_LE(planner_runtime + t0, operating_point_->t0 + time::getTimeHorizon());
  ...
  Time remaining_time_this_step =
      (current_timestep + 1) * time::getTimeStep() - relative_t0;
  if (remaining_time_this_step < kRoundingError * time::getTimeStep()) {
    current_timestep += 1;
    remaining_time_this_step = time::getTimeStep() - remaining_time_this_step;
  }

  CHECK_LT(remaining_time_this_step, time::getTimeStep());
  ...
  if (remaining_time_this_step <= planner_runtime) {
    const size_t num_steps_to_integrate = static_cast<size_t>(
        ...
        (planner_runtime - remaining_time_this_step) / time::getTimeStep());
    ...
    op.t0 += time::getTimeStep() * num_steps_to_integrate;
  }
  ...
  // Check an invariant.
  CHECK_LE(std::abs(t0 + planner_runtime - op.t0), time::getTimeStep());
...
void Problem::SetUpNextRecedingHorizon(const VectorXf& x0, Time t0,
  ...
  const size_t after_final_timestep =
      first_timestep_in_new_problem + time::getNumTimeSteps();
  ...
  CHECK_GE(operating_point_->xs.size(), time::getNumTimeSteps());
  if (operating_point_->xs.size() > time::getNumTimeSteps()) {
    operating_point_->xs.resize(time::getNumTimeSteps());
    operating_point_->us.resize(time::getNumTimeSteps());
    for (PlayerIndex ii = 0; ii < dynamics_->NumPlayers(); ii++) {
      (*strategies_)[ii].Ps.resize(time::getNumTimeSteps());
      (*strategies_)[ii].alphas.resize(time::getNumTimeSteps());
    }
  }
  ...
  for (size_t kk = timestep_iterator_end - first_timestep_in_new_problem;
    ...
    operating_point_->xs[kk] = dynamics_->Integrate(
        RelativeTimeTracker::RelativeTime(kk - 1), time::getTimeStep(),
