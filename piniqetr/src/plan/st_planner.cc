...
PlannerStatus StPlanner(
  ...
  SpacetimeTrajectoryManager st_traj_mgr_ilq;
  if (input.ilq_games_params->enable_ilq_games()) {
    Timer start_time;
    st_traj_mgr_ilq = BuildILQGames(
        {.psmm = input.planner_semantic_map_manager,
         .passage = &decision_exploration.plan_passage,
         .lc_stage = decision_exploration.lane_change_state.stage(),
         .veh_geom = &vehicle_geom_params,
         .plan_start_point = &input.start_point_info->start_point,
         .st_traj_mgr = input.st_traj_mgr,
         .ilq_games_params = input.ilq_games_params,
         .seq_num = input.seq_num});
    Log2FG::LogDataV0("ilq_games_time_cost(ms)",
                      std::to_string(start_time.TimeNs() / 1E6));
  }
  ...
  RETURN_PLANNER_STATUS_OR_ASSIGN(
      ...
      ExecuteSpeedPlanning(
          {.base_name = "st",
           .behavior = input.behavior,
           .psmm = input.planner_semantic_map_manager,
           .traj_mgr = input.ilq_games_params->enable_ilq_games()
                           ? &st_traj_mgr_ilq
                           : input.st_traj_mgr,
