...
constexpr double kPerceptionObjectToAVTime = 1.5;
  ...
  for (const auto& seg : lane_path_from_start) {
    ...
    for (const auto& cw_id : lane_info.crosswalks()) {
      ...
      crosswalk_managers.emplace_back(CrosswalkManager{
          ...
          .tl_controlled = has_traffic_light,
...
absl::StatusOr<CrosswalkDescriptor> BuildCrosswalkConstraints(
  ...
  for (const auto& cw_manager : crosswalk_managers) {
    ...
    for (const auto& object : obj_mgr.planner_objects()) {
      ...
      const double vx = object.pose().v() * std::cos(object.pose().theta());
      const double vy = object.pose().v() * std::sin(object.pose().theta());

      const Vec2d agent_velocity_vector(vx, vy);

      const double perc_object_signed_velocity_proj_on_cw =
          agent_velocity_vector.dot(cw_axis_unit_dir);
      ...
      const double percobj_t_at_av =
          object_lead_av_dist /
          std::max(std::abs(perc_object_signed_velocity_proj_on_cw), kEpsilon);

      if (!same_side && percobj_t_at_av > kPerceptionObjectToAVTime) {
        continue;
      }
