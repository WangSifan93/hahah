...
void CartesianFrenetConverter::cartesian_to_frenet(
  ...
  const auto angle_rtheta = math::Angle16::FromRad(rtheta);
  const double cos_theta_r = math::cos(angle_rtheta);
  const double sin_theta_r = math::sin(angle_rtheta);
  ...
  const auto angle_delta_theta = math::Angle16::FromRad(delta_theta);
  const double tan_delta_theta = math::tan(angle_delta_theta);
  const double cos_delta_theta = math::cos(angle_delta_theta);
...
void CartesianFrenetConverter::frenet_to_cartesian(
  ...
  *ptr_y = ry + cos_theta_r * d_condition[0];
  ...
  const double cos_delta_theta = math::cos(math::Angle16::FromRad(delta_theta));
...
Vec2d CartesianFrenetConverter::CalculateCartesianPoint(const double rtheta,
                                                        ...
                                                        const double l) {
  const auto angle_rtheta = math::Angle16::FromRad(rtheta);
  const double x = rpoint.x() - l * math::sin(angle_rtheta);
  const double y = rpoint.y() + l * math::cos(angle_rtheta);
  return {x, y};
}
...
double CartesianFrenetConverter::CalculateSecondOrderLateralDerivative(
  ...
  const auto angle_diff = math::Angle16::FromRad(theta_diff);
  const double cos_theta_diff = math::cos(angle_diff);
  const double res = -(rdkappa * l + rkappa * dl) * math::tan(angle_diff) +
