#include "math/geometry/affine_transformation.h"
...
Vec3d ConvertSkewMatrixToVector(const Mat3d& u_cross) {
...
Mat3d BuildRotationFromAxisAngle(double cos_theta, double sin_theta,
                                 const Vec3d& u) {
...
void AngleAndUnitAxisFromRotationMatrix(const Mat3d& rot, double* theta,
  ...
  CHECK(theta != nullptr);
  CHECK(u != nullptr);
  const Vec3d axis = ConvertSkewMatrixToVector(rot - rot.transpose());
...
AffineTransformation& AffineTransformation::SetRotation(double theta,
  ...
  mat_.block<3, 3>(0, 0) =
      BuildRotationFromAxisAngle(cos_theta, sin_theta, normalized_axis);
  return *this;
}
...
AffineTransformation& AffineTransformation::SetRotation(Quaternion quaternion) {
  ...
  mat_.block<3, 3>(0, 0) =
      BuildRotationFromAxisAngle(cos_theta, sin_theta, axis);
  return *this;
}
