#include "common/ref_line/ref_routing_lane_center.h"
...
void RefRoutingLaneCenter::Init(const Vec2d& pos, MapConstPtr map,
  ...
  ego_current_lane =
      map->GetNearestLane(pos, FLAGS_ad_e2e_current_lane_search_dist);
  if (ego_current_lane == nullptr) {
    LERROR("adc not on lane, fail to build ref routing lane center");
    return;
  }

  std::unordered_set<std::string> all_routing_lane_set;
  if (route != nullptr) {
  ...
  std::unordered_set<std::string> iter_lanes{ego_current_lane->id()};
  ...
  while ((!iter_lane->valid_next_lane_ids().empty()) && (!is_find_loop)) {
    ...
    for (auto suc_lane_id : iter_lane->valid_next_lane_ids()) {
      ...
      if (iter_heading_ok && suc_heading_ok) {
        angle_diff = std::abs(
            ad_e2e::planning::math::AngleDiff(suc_heading, iter_heading));
      } else {
        angle_diff = 0.0;
      }
  ...
  if (!route_lanes.empty()) {
    ...
    route_lanes.back()->GetHeadingFromS(route_lanes.back()->curve_length(),
                                        &end_heading);
    is_straight_flag_ = std::fabs(ad_e2e::planning::math::AngleDiff(
                            end_heading, begin_heading)) < m_pi_3_;
  }
