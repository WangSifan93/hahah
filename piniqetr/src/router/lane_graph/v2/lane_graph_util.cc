...
constexpr double kSplitFactor = 1.1;
...
double GetLaneKeepCost(const ad_e2e::planning::Map& smm,
  ...
  double split_factor = lane_info.is_split() ? kSplitFactor : 1.0;
  double lane_cost = split_factor * kLaneLengthCost * sec_info.curve_length() *
                     (to_frac - from_frac);
