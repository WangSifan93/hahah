#ifndef AD_E2E_PLANNING_NODES_SUBSCRIBER_H
#define AD_E2E_PLANNING_NODES_SUBSCRIBER_H
...
#if 0
        LOG(ERROR) << this->GetTopic() << " stamp repeats " << std::fixed
                   << std::setprecision(4) << *prev_stamp << " -> "
                   << next_stamp;
#endif
        queue_.Clear();
      } else if (math::Double::CompareType::GREATER == comp) {
        LOG(ERROR) << this->GetTopic() << " stamp goes backward: " << std::fixed
                   << std::setprecision(4) << *prev_stamp << " -> "
                   << next_stamp;
        queue_.Clear();
      }
