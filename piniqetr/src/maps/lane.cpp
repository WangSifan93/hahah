...
void Lane::GetWidthAtAccumS(const double &s, double &left_w,
...
double Lane::CalculateWidth(math::LineSegment2d &base, Point2d &p,
                            int lr) const {
...
bool Lane::IsPrecede(const std::string &lane_id) const {
  auto it = std::find(prev_lane_ids_.begin(), prev_lane_ids_.end(), lane_id);
  return it != prev_lane_ids_.end();
}

bool Lane::IsNext(const std::string &lane_id) const {
...
bool Lane::GetWidthFromS(const double &query_s, double *const lw,
...
double Lane::LaneFraction(int segment, double segment_fraction) const {
  ...
  return e2e_noa::Lerp(center_line_.GetAccuLength()[segment],
                       center_line_.GetAccuLength()[segment + 1],
                       segment_fraction) /
         center_line_.GetAccuLength().back();
}

std::pair<int, double> Lane::SegmentFraction(double lane_fraction) const {
  CHECK_GE(center_line_.GetAccuLength().size(), 2);
  CHECK_GE(lane_fraction, 0.0);
  CHECK_LE(lane_fraction, 1.0);
  const double cum_length = center_line_.GetAccuLength().back() * lane_fraction;
  int index = std::upper_bound(center_line_.GetAccuLength().begin(),
                               center_line_.GetAccuLength().end(), cum_length) -
              center_line_.GetAccuLength().begin();
  CHECK_GT(index, 0);
  if (index == center_line_.GetAccuLength().size()) {
    return {center_line_.GetAccuLength().size() - 2, 1.0};
  }
  --index;
  CHECK_LT(index + 1, center_line_.GetAccuLength().size());
  return {index, (cum_length - center_line_.GetAccuLength()[index]) /
                     (center_line_.GetAccuLength()[index + 1] -
                      center_line_.GetAccuLength()[index])};
}
...
math::Vec2d Lane::LerpPointFromFraction(double fraction) const {
  ...
  return e2e_noa::Lerp(points()[index.first], points()[index.first + 1],
                       index.second);
}
