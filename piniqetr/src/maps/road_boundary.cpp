...
namespace {
constexpr bool kEnalbeInterpolate = true;
}
...
void RoadBoundary::InterpolatePoints() {
  ...
  const double map_gap_threshold_sqr =
      FLAGS_ad_e2e_planning_map_point_distance_threshold *
      FLAGS_ad_e2e_planning_map_point_distance_threshold;
  for (int32_t i = 1; i < static_cast<int32_t>(points_.size()); ++i) {
    ...
    int32_t insert_cnt = static_cast<int32_t>(
        cur_length / FLAGS_ad_e2e_planning_boundary_interpolate_dist);
    const auto norm_vector =
        (cur_pt - last_pt) /
        (cur_length + FLAGS_ad_e2e_planning_zero_threshold) *
        FLAGS_ad_e2e_planning_boundary_interpolate_dist;
    for (int32_t j = 0; j < insert_cnt; ++j) {
      const auto insert_point = last_pt + norm_vector * (j + 1);
      interpolate_points.push_back(insert_point);
    }
