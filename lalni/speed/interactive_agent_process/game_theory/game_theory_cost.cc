...
void GameTheoryCost::judge_ego_obj_responsibility(
    ...
    const TrajectoryPointWithAcceleration &ego_center_pose,
    const TrajectoryPointWithAcceleration &obj_center_pose,
...
lon_dir_t GameTheoryCost::CalAgentsRelaLonDir(const FrenetBox &frenet_box_a,
  ...
  constexpr double theta_threshold =
      ad_e2e::planning::Constants::DEG2RAD * 90.0;

  if (dz_fgreater(dz_fabs(ego_theta - obj_theta), theta_threshold)) {
    return LD_FRONT;
  }
...
void GameTheoryCost::CalRssLateralSafeDist(
  ...
  double result = 0.0;
  double ego_lat_brake_max = dz_fequal(ego_lateral_brake_max, 0.0)
                                 ? FLOAT_EPSILON
                                 : ego_lateral_brake_max;
  double ego_lat_brake_min = dz_fequal(ego_lateral_brake_min, 0.0)
                                 ? FLOAT_EPSILON
                                 : ego_lateral_brake_min;
  double agent_lat_brake_max = dz_fequal(agent_lateral_brake_max, 0.0)
                                   ? FLOAT_EPSILON
                                   : agent_lateral_brake_max;
  double agent_lat_brake_min = dz_fequal(agent_lateral_brake_min, 0.0)
                                   ? FLOAT_EPSILON
                                   : agent_lateral_brake_min;

  double ego_lat_vel_abs = dz_fabs(ego_vel);
  double agent_lat_vel_abs = dz_fabs(agent_vel);
  ...
  if (LAT_RIGHT == lat_direct) {
    if (dz_fless(ego_vel, 0.0) && dz_fless(agent_vel, 0.0)) {
      result = ego_passive_brake_distance - agent_active_brake_distance;
    } else if (dz_fless(ego_vel, 0.0) && !dz_fless(agent_vel, 0.0)) {
...
void GameTheoryCost::CalRssLonSafeVelBoundary(
  ...
  double agent_abs_vel = dz_fabs(agent_vel);
  ...
  if (LD_FRONT == lon_direct) {
    if (!dz_fless(agent_vel, 0.0)) {
      ...
      *ego_vel_low = 0.0;
      ...
      if (dz_fgreater(agent_driven_distance, lon_abs_distance)) {
        *ego_vel_high = 0.0;
        *ego_vel_low = 0.0;
      } else {
        double a = 1.0 / (2.0 * longitudinal_brake_min);
    ...
    if (!dz_fless(agent_vel, 0.0)) {
      ...
      if (dz_fless(agent_driven_distance, lon_abs_distance)) {
...
void GameTheoryCost::GameTheoryRssSafeCheckEgo(
  ...
  double ego_agent_lateral_abs_dist = dz_fabs(ego_state->frenet_box.center_l() -
  ...
  if (!dz_fless(ego_agent_lateral_abs_dist, safe_lat_dist)) {
    *is_safe = true;
    *lon_violate_type = LON_VIO_LEGAL;
    *rss_vel_low = 0.0;
    *rss_vel_high = 0.0;
  ...
  if (!dz_fgreater(lon_relative_distance, 0.0) && (LD_FRONT == lon_dir)) {
    *is_safe = false;
  ...
  if (dz_fgreater(ego_state->sl_vel.s_dt, ego_vel_high)) {
    *is_safe = false;
    *lon_violate_type = LON_VIO_TOOFAST;
    *rss_vel_low = ego_vel_low;
    *rss_vel_high = ego_vel_high;
  } else if (dz_fless(ego_state->sl_vel.s_dt, ego_vel_low)) {
    *is_safe = false;
...
void GameTheoryCost::GameTheoryRssSafeCheckObj(
  ...
  safe_lat_dist += 0.5 * (ego_width + obj_width);
  double agent_ego_lateral_abs_dist = dz_fabs(ego_state->frenet_box.center_l() -
                                              obj_state->frenet_box.center_l());
  ...
  if (!dz_fless(agent_ego_lateral_abs_dist, safe_lat_dist)) {
    *is_safe = true;
    *lon_violate_type = LON_VIO_LEGAL;
    *rss_vel_low = 0.0;
    *rss_vel_high = 0.0;
  ...
  if (dz_fless(obj_state->sl_vel.s_dt, 0.0)) {
  ...
  if (dz_fgreater(temp_vel, obj_vel_high)) {
    *is_safe = false;
    *lon_violate_type = LON_VIO_TOOFAST;
    *rss_vel_low = obj_vel_low;
    *rss_vel_high = obj_vel_high;
  } else if (dz_fless(temp_vel, obj_vel_low)) {
    *is_safe = false;
    *lon_violate_type = LON_VIO_TOOSLOW;
    *rss_vel_low = obj_vel_low;
    *rss_vel_high = obj_vel_high;
...
void TurnLeftCrossCost::EgoTrafficRuleCost(
  ...
  if (is_collision && is_ego_responsibility) {
    if (dz_fequal(ego_state->vel, 0.0)) {
      if (!dz_fless(ego_state->wait_time, 2.0)) {
...
void TurnLeftCrossCost::EgoAccelerateCost(
  ...
  double delta_acc = dz_fabs(ego_init_acc - ego_sampling_acc);
  double ratio = 1.0;
  if (obj_init_acc > 0.0) {
    ratio = 1.0;
  } else {
  ...
  if (StBoundaryProto::OVERTAKE == decision_type) {
    if (dz_fless(ego_state->vel, obj_state->vel) &&
        dz_fgreater(ego_sampling_acc, ego_init_acc)) {
      acc_cost = 0.0;
    } else {
      if (dist_ratio < 2.5) {
    ...
    if (dz_fgreater(ego_state->vel, obj_state->vel) &&
        dz_fless(ego_sampling_acc, ego_init_acc)) {
      acc_cost = 0.0;
    } else {
      acc_cost = 0.01 * ego_state->vel * std::pow(2 * delta_acc, 2);
      acc_cost *= ratio;
  ...
  if (!is_rss_safe) {
    if (LON_VIO_TOOFAST == lon_violate_type) {
      step_obj_cost =
          ...
          std::pow(10, static_params.rss_params.over_speed_power_coeff *
                           dz_fabs(obj_state->vel - rss_vel_high));
      if (is_debug_cost) {
        printf("[ obj_safety_cost-  F ]: delta_vel %.2f step_obj_cost %.2f\n",
               dz_fabs(obj_state->vel - rss_vel_high) * 3.6, step_obj_cost);
      }
    } else {
      step_obj_cost =
          static_params.rss_params.lack_speed_linear_coeff * obj_state->vel *
          std::pow(10, static_params.rss_params.lack_speed_power_coeff *
                           dz_fabs(obj_state->vel - rss_vel_low));
      if (is_debug_cost) {
        printf("[ obj_safety_cost-  S ]: delta_vel %.2f step_obj_cost %.2f\n",
               dz_fabs(obj_state->vel - rss_vel_low) * 3.6, step_obj_cost);
      }
    }
    step_obj_cost =
...
void TurnLeftCrossCost::ObjAccelerateCost(
    ...
    const double ego_dist_to_enter_intersection,
    const double obj_dist_to_enter_intersection) {
  double delta_acc = dz_fabs(obj_init_acc - obj_sampling_acc);
  ...
  if (obj_init_acc > 0 && !dz_fgreater(dist_ratio, 3.0)) {
    if (dz_fequal(obj_state->vel, 0.0)) {
      acc_cost = std::pow(2 * delta_acc, 2);
    }
  }
