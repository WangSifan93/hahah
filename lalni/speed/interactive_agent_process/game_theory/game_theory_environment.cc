...
void Game_Theory_Interaction_Environment::DumpCostMatrixSimple(
  ...
  for (j = 0; j < ego_acc_size; j++) {
    for (i = 0; i < obj_acc_size; i++) {
      ...
      if (0 == i) {
        if (!dz_fless(gt_interaction_matrix->Matrix[j][i].ego_acc, 0.0)) {
          printf("+");
        }
...
Game_Theory_Interaction_Environment::UpdateEgoActionSimState(double ego_acc) {
  ...
  for (int i = 0; i < (MAX_STATE_NUM - 1); i++) {
    ...
    if (!dz_fless(next_state.time_stamp,
                  this->static_params_.max_simulation_time) ||
        !dz_fless(next_state.dist, this->static_params_.max_simulation_dist)) {
      break;
    }
  }
...
void Game_Theory_Interaction_Environment::UpdateDecisionResult(
  ...
  if (dz_fequal(inter_data->ego_acc, target_ego_acc) &&
      dz_fequal(inter_data->agent_acc, target_obj_acc)) {
    is_debug = true;
  }
#endif
  if (is_debug) {
...
#if DEBUG_SINGLE
  if (dz_fequal(ego_target_acc, inter_data->ego_acc) &&
      dz_fequal(obj_target_acc, inter_data->agent_acc)) {
    is_debug_cost = true;
  }
#endif
...
void Game_Theory_Interaction_Environment::CalcEgoMaxAccBySpeedLimit(
  ...
  if (dz_fgreater(ego_dist_to_collision_pt, 0.0)) {
  ...
  if (dz_fgreater(ego_dist_leave_collision_pt, 0.0)) {
    ComputeExptAcc(&ego_end_calc_acc, ego_vel, ego_leave_collision_max_vel,
                   ego_dist_leave_collision_pt);
  }

  if (dz_fless(ego_end_calc_acc, std::numeric_limits<double>::max()) &&
      dz_fless(ego_start_calc_acc, std::numeric_limits<double>::max())) {
    *ego_max_acc = (ego_start_calc_acc + ego_end_calc_acc) / 2.0;
  } else if (!dz_fless(ego_end_calc_acc, std::numeric_limits<double>::max()) &&
             !dz_fless(ego_start_calc_acc,
