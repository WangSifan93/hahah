...
void CvtPoseGlobalToLocal(TrajectoryPointWithAcceleration *local_pose,
  ...
  double tmp_theta = ad_e2e::planning::math::NormalizeAngle(
...
void CvtPoseLocalToGlobal(TrajectoryPointWithAcceleration *global_pose,
...
void JudgeTurnInfo(bool *is_turn_left, bool *is_turn_right, bool *is_uturn,
  ...
  constexpr double kTurnHeadingDiff = M_PI / 3.0;
...
void InteractiveAgent::SetEgoInitState(
  ...
  this->SetPlanPassage(plan_passage);
  ...
  if (av_shapes.size() > 0) {
    ...
    if (nullptr != av_shape_ptr) {
      ego_init_polygon = Polygon2d(
          av_shape_ptr->GetCornersWithBufferCounterClockwise(0.2, 0.2), true);
    }
  }
  ...
  for (int i = 0; i < path.size(); i++) {
    ...
    TrajectoryPointWithAcceleration sl_center_pose, global_ego_pose,
        local_center_pose;
    ...
    auto frenet_box = plan_passage->QueryFrenetBoxAtContour(av_polygon, false);
...
void InteractiveAgent::TransformGlobalToSlCenterPose(
  ...
  if (nullptr == plan_passage) return;

  int cur_station_index =
      plan_passage->FindNearestStationIndex(global_center_pose.pos()).value();

  const Station &cur_station =
      plan_passage->station(StationIndex(cur_station_index));

  double theta = cur_station.tangent().FastAngle();
  TrajectoryPointWithAcceleration target_pose;
...
void InteractiveAgent::CvtGlobalPoseToSlCenterPose(
  ...
  if (nullptr == sl_center_pose || nullptr == global_center_pose ||
      nullptr == this->plan_passage())
    return;

  TrajectoryPointWithAcceleration local_center_pose;
  TransformGlobalToSlCenterPose(&local_center_pose, *global_center_pose,
                                this->plan_passage(), is_debug);

  auto frenet_pos =
      this->plan_passage()->QueryFrenetCoordinateAt(global_center_pose->pos());
...
void InteractiveAgent::SetAgentInitState(
    const SpacetimeObjectTrajectory *obstacle, const PlanPassage *plan_passage,
  ...
  if (nullptr == obstacle || nullptr == plan_passage ||
      nullptr == st_boundary_decision)
    return;

  is_valid_ = true;
  this->SetAgentID(obstacle->object_id());
  ...
  for (int i = 0; i < obstacle->states().size(); i++) {
    ...
    TrajectoryPointWithAcceleration global_obj_pose, sl_center_pose;
    ...
    auto frenet_box = plan_passage->QueryFrenetBoxAtContour(
...
void InteractiveAgent::GetEgoPolygonAccordingToDist(
    Polygon2d *ego_polygon, TrajectoryPointWithAcceleration *global_center_pose,
  ...
  TrajectoryPointWithAcceleration *low_point, *upper_point;
  ...
  TrajectoryPointWithAcceleration target_point;
  ...
  *ego_polygon = Polygon2d(
      target_ego_shape.GetCornersWithBufferCounterClockwise(0.2, 0.2), true);
}
...
void InteractiveAgent::GetEgoStateInfoAccordingToDist(
  ...
  if (nullptr == plan_passage() || nullptr == ego_polygon) return;

  auto tmp_frenet_box =
      plan_passage()->QueryFrenetBoxAtContour(*ego_polygon, false);
...
void InteractiveAgent::GetAgentPolygonAccordingToDist(
    Polygon2d *obj_polygon, TrajectoryPointWithAcceleration *global_center_pose,
    int *traj_point_idx, const double dist, int start_idx) {
...
void InteractiveAgent::GetAgentStateInfoAccordingToDist(
  ...
  auto tmp_frenet_box =
      plan_passage()->QueryFrenetBoxAtContour(*obj_polygon, false);
  if (tmp_frenet_box.ok()) {
    *frenet_box = tmp_frenet_box.value();
  }
}
...
void InteractiveAgent::UpdateGameTheoryScenario(const bool ego_turn_left,
  ...
  TrajectoryPointWithAcceleration global_start, global_end;
...
void InteractiveAgent::SetMovAgentParams(
  ...
  if (nullptr == obstacle) return;

  obstacle_ = obstacle;

  ObjectType object_type = obstacle->object_type();
  ...
  if (nullptr != st_boundary_decision_ &&
      nullptr != st_boundary_decision_->raw_st_boundary()) {
    auto obj_sl_info = st_boundary_decision_->raw_st_boundary()
                           ->obj_scenario_info()
                           .obj_sl_info;
    double obj_length = std::fabs(obj_sl_info.frenet_polygon.s_max -
                                  obj_sl_info.frenet_polygon.s_min);
    double obj_width = std::fabs(obj_sl_info.frenet_polygon.l_max -
                                 obj_sl_info.frenet_polygon.l_min);
