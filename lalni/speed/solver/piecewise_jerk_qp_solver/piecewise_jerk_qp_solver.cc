...
#include "speed/solver/qp/osqp_return_status.h"
...
absl::Status PiecewiseJerkQpSolver::OptimizeWithOsqp(
    ...
    std::vector<c_float>& A_data, std::vector<c_int>& A_indices,
    std::vector<c_int>& A_indptr, std::vector<c_float>& lower_bounds,
    std::vector<c_float>& upper_bounds, std::vector<c_float>& q, OSQPData* data,
  ...
  LOG(INFO) << "num_affine_constraint = " << num_affine_constraint;
  LOG(INFO) << "status = " << status_info;
