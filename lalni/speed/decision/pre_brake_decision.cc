...
constexpr double kMildAccel = 0.2;

constexpr double kPedestrainStandoffFollowDist = 2.0;
constexpr double kPedestrainEmergencyBrakeAccel = -2.3;
constexpr double kPedestrainEmergencyBrakeTime = 2.0;
constexpr double kPedestrainEmergencyBrakeVel =
    -kPedestrainEmergencyBrakeAccel * kPedestrainEmergencyBrakeTime;
constexpr double kPedestrainEmergencyBrakeDist =
    0.5 * kPedestrainEmergencyBrakeVel * kPedestrainEmergencyBrakeTime +
    kPedestrainStandoffFollowDist;
...
constexpr double kCyclistStandoffFollowDist = 2.0;
constexpr double kCyclistEmergencyBrakeAccel = -1.0;
constexpr double kCyclistEmergencyBrakeTime = 1.0;
constexpr double kCyclistEmergencyBrakeVel =
    -kCyclistEmergencyBrakeAccel * kCyclistEmergencyBrakeTime;
constexpr double kCyclistEmergencyBrakeDist =
    0.5 * kCyclistEmergencyBrakeVel * kCyclistEmergencyBrakeTime +
    kCyclistStandoffFollowDist;

constexpr double kVehicleStandoffFollowDist = 3.0;
constexpr double kVehicleEmergencyBrakeAccel = -1.0;
constexpr double kVehicleEmergencyBrakeTime = 0.5;
constexpr double kVehicleEmergencyBrakeVel =
    -kVehicleEmergencyBrakeAccel * kVehicleEmergencyBrakeTime;
constexpr double kVehicleEmergencyBrakeDist =
    0.5 * kVehicleEmergencyBrakeVel * kVehicleEmergencyBrakeTime +
    kVehicleStandoffFollowDist;
constexpr double kObjectReactionTime = 1.0;
constexpr double kObjectReactionTimeForFov = 0.4;
...
std::optional<VtSpeedLimit> MakePedestrianPreBrakeDecisionForStBoundary(
  ...
  if (params.ignore_late_cut_in_ped()) {
    ...
    constexpr double kPedPreBrakeMaxTime = 6.0;

    if (ped_t > kPedPreBrakeMaxTime) {
      const auto info = absl::StrCat(
          "Ignore all pedestrains that overlap with AV's path after 6s ",
          *traj_id);
      return GenerateConstAccSpeedLimit(0.0, kPedestrainEmergencyBrakeTime,
                                        current_v, max_v, max_v, 0.0, time_step,
                                        step_num, info);
    }
  }
  ...
  if (Relationship::Cross ==
      ...
      InteractionZone::TurnLeft ==
          st_boundary_wd.st_boundary()->obj_scenario_info().interaction_zone) {
  ...
  constexpr double kMildAccTimeLimit = 5.0;
  ...
  return GenerateConstAccSpeedLimit(
      0.0, pre_brake_time + kPedestrainEmergencyBrakeTime, current_v, 0.0,
...
bool IsUncertainVehicle(const SpacetimeTrajectoryManager& st_traj_mgr,
  ...
  *is_av_completely_in_object_fov = IsAvCompletelyInObjectFov(
  ...
  const auto av_ttc_info = GetAvOverlapTtcInformation(
      overlap_infos, overlap_meta, path, preliminary_speed);
...
std::optional<VtSpeedLimit> GenerateUncertainVehicleSpeedLimit(
  ...
  return GenerateConstAccSpeedLimit(0.0, pre_brake_time + emergency_brake_time,
                                    current_v, min_v, max_v, pre_brake_acc,
                                    time_step, step_num, info);
}
...
std::optional<VtSpeedLimit> MakePedestrainPreBrakeDecision(
  ...
  for (auto& st_boundary_wd : *st_boundaries_wd) {
    std::optional<VtSpeedLimit> speed_limit_opt =
        MakePedestrianPreBrakeDecisionForStBoundary(params, st_traj_mgr, path,
    ...
    if (speed_limit_opt.has_value()) {
      ...
      MakeIgnoreDecision(
          absl::StrCat(st_boundary_wd.id(),
                       " is ignored by pedestrian pre-brake decision"),
      ...
      for (auto* modified_st_boundary_wd : modified_st_boundaries_wd) {
        MakeIgnoreDecision(
            absl::StrCat(modified_st_boundary_wd->id(),
                         " is ignored by uncertain vehicle pre-brake decision"),
...
bool IsCreepInteractionVehicle(const SpacetimeTrajectoryManager& st_traj_mgr,
  ...
  if (obj_scene_info.interaction_zone != InteractionZone::JunctionStraight) {
#if DEBUG_CREEP_INTERACTION
